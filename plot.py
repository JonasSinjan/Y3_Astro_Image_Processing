...
if __name__ == '__main__':
    def main(filename, filename2, filename3, filename4):
        ...
        for count, mag in enumerate(mag_list):
            ...
            #plt.plot(m, [i * slope + intercept for i in m], color=color_list[count+1], label=fit_str)
            filename = file_list[count]
            plt.plot(m, np.log10(N), color=color, marker='.', label=f'{sig_list[count],round(slope, 3)}')
            plt.xlabel('m')
            plt.ylabel('Log10(N)')
            plt.legend()
        plt.show()
    ...
    #     img.trim(150)
    #     fig, ax = plt.subplots(figsize=(6, 11), dpi=800)
    #     image_map = ax.imshow(img.data * img.mask, origin="lower", cmap='binary',aspect="equal")
    #
    #     plt.show()
    #
    #     df = pd.read_csv(filename)
    ...
    #main_thread = threading.Thread(target=main, args=('survey_5sig_0.85_new.cat',))
    main_thread.start()
